    ...
    for sel in search_selectors:
        try:
            ...
            if await input_el.count() > 0 and await input_el.is_visible():
                ...
                await input_el.fill(term)
                await page.wait_for_timeout(300)
                await input_el.press("Enter")
                sys.stderr.write(f"[COURSEU] Searched for: {term}\n")
                return True
...
async def extract_price(page) -> tuple:
    """Extract price and product info from page"""
    price = None
    title = None
    unit_price = None
    quantity = None
    ...
    try:
        ...
        # Get unit price
        unit_selectors = [".product-price__unit", ".prix-unitaire"]
        for sel in unit_selectors:
            try:
                unit_el = page.locator(sel).first
                if await unit_el.count() > 0:
                    unit_price = await unit_el.inner_text()
                    break
            except Exception:
                pass
        ...
        # Get image
        try:
            img_el = page.locator("img.product-image, .product-visual img").first
    ...
    return price, title, unit_price, quantity, image
...
async def collect() -> Result:
    ...
    try:
        ...
        price, title_text, unit_price, quantity, image = await extract_price(page)
        ...
        if price:
            return Result(
                ...
                store=STORE_NAME,
                matched_ean=EAN if EAN in (await page.content()) else None,
                image=image,
            )
        else:
            return Result(
