#!/usr/bin/env python3
"""Fetcher Casino Shop (mescoursesdeproximite.com) via requêtes HTTP."""
...
STORE_SLUG = os.environ.get("CASINO_STORE_SLUG", "casino-shop-33130").strip()
STORE_LABEL = os.environ.get("CASINO_STORE_LABEL", "Casino Shop · Bègles Pruniers").strip()
STORE_PATH = f"{STORE_SLUG}/{STORE_CODE}".strip("/")
STORE_URL = os.environ.get("CASINO_STORE_URL") or f"{BASE_URL.rstrip('/')}/courses-en-ligne/{STORE_PATH}"
SEARCH_TEMPLATE = os.environ.get(
    "CASINO_SEARCH_TEMPLATE",
...
def fetch_casino() -> ResultPayload:
    if not EAN:
    ...
    if response.status_code == 404:
        return ResultPayload(status="NO_RESULTS", note="Casino Shop (404)", _meta=meta)
    ...
    if not candidates:
        return ResultPayload(status="NO_RESULTS", note="Casino Shop", _meta=meta)
