#!/usr/bin/env python3
"""Wrapper qui impose la méthode Carrefour City pour tout fetch.

La logique détaillée (trace obligatoire, magasin attendu, champs de sortie)
est décrite dans ``collection_mandate.METHODS['carrefour_city']``. Ne jamais
modifier ce workflow sans accord explicite de Laurent.
"""
import argparse
import json
import os
...
MANDATE = get_method("carrefour_city")
...
def run(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(description='Fetch Carrefour City price via CDP (City Bordeaux Balguerie).')
