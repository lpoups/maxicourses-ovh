...
class LeclercAdapter:
    ...
    def hard_validate(self, src: ProductDescriptor, url: str, pd: ProductDescriptor) -> Optional[float]:
        ...
        if ean:
            ...
            # Utiliser le image_provider Playwright (comme Monoprix) pour bypass CORS/403
            img_provider = self.image_compare()
            if img_provider:
                try:
                    if img_provider(src.image_url, pd.image_url):
                        pd.ean = src.ean
                        return 0.92
                except Exception:
                    pass
            # Fallback sur compare_references si provider non disponible
            try:
...
class FinderPipeline:
    ...
    def ensure_hooks(cls) -> None:
        ...
        try:
            ...
            # Utiliser le même image provider que Monoprix pour Leclerc
            leclerc_image_provider = _make_monoprix_image_provider()
            if leclerc_image_provider is not None:
                LeclercAdapter._image_provider = leclerc_image_provider  # type: ignore[attr-defined]
        except Exception:
            pass
        try:
