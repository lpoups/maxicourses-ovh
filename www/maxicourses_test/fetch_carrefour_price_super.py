#!/usr/bin/env python3
"""Wrapper appliquant la méthode Carrefour Super (Lormont)."""
...
MANDATE = get_method("carrefour_super")
TRACE_FILES = [ROOT.parent / "traces" / name for name in MANDATE.trace_files if name]
...
def run(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(description="Fetch Carrefour Super price via CDP (Super Lormont).")
    ...
    env["CARREFOUR_STATE_VARIANT"] = os.getenv("CARREFOUR_SUPER_STATE", "carrefour_super")

    default_frontal = os.environ.get("CARREFOUR_SUPER_FRONTAL_STORE") or os.environ.get("CARREFOUR_FRONTAL_STORE")
    if default_frontal:
        env["CARREFOUR_FRONTAL_STORE"] = default_frontal
    frontal_store = env.get("CARREFOUR_FRONTAL_STORE")

    if TRACE_FILES and not frontal_store:
        replay_traces(env)
