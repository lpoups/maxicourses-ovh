...
- **Fait** : Leclerc Drive passe en mode « PDP cache » : à la première collecte on mémorise l’URL produit dans `manual_descriptors.json` (`leclerc_url`), puis les runs suivants sautent la recherche et charge directement la fiche (`fetch_leclerc_drive_price.py` / `manual_leclerc_cdp.py`). Résultat : exécution ≤3 s (`results/run-3124480200433-20251030-160830.json`).
