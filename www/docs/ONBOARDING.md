...
- **Procédure de collecte (sans Leclerc)** :
 ...
 2. Pour chaque EAN : `./maxicourses_test/run_pipeline_server.sh <EAN> --adapters carrefour_city carrefour_market carrefour_super auchan chronodrive courseu g20 casino spar intermarche monoprix`.
...
- **Gel fetchers existants** : ne toucher sous aucun prétexte aux scripts `fetch_carrefour_price_market.py`, `fetch_carrefour_price_city.py`, `fetch_auchan_price.py`, `fetch_chronodrive_price.py`, `fetch_courseu_price.py`, `fetch_casino_price.py`, `fetch_intermarche_price.py`, `fetch_leclerc_drive_price.py`, `fetch_monoprix_price.py` (ni à leurs helpers) tant que Laurent n’a pas validé une modification explicite. Seul le wrapper `fetch_carrefour_price_super.py` peut évoluer sans validation préalable (objectif : sécuriser la collecte Carrefour Super Lormont). Le fetch Spar s’appuie sur `fetch_casino_price.py` (même script, autre magasin) et reste gelé au même titre.
...
1. **Collecte seed systématique** : commencer chaque produit par une recherche **100 % EAN brut** (sans texte additionnel) sur les enseignes qui l’acceptent :
   ...
   - Spar Super Saint-Médard-en-Jalles (même procédure que Casino).
