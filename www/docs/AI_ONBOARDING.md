...
### Gestion des Images (Phase 8 - Déc 2025)

> [!IMPORTANT]
> Les images sont stockées **localement sur OVH** (`pipeline/assets/`) et référencées dans MongoDB.
> **INTERDIT** : URLs externes des magasins (peuvent expirer).

```python
from descriptor_store import (
    save_product_image,
    download_and_save_image,
    ensure_product_has_image,
    sync_all_images_to_mongodb
)

# Sauvegarder une image (bytes → fichier local + MongoDB)
save_product_image("5000112611861", image_bytes, "jpg")

# Télécharger et sauvegarder depuis une URL
download_and_save_image("5000112611861", "https://exemple.com/image.jpg")

# S'assurer qu'un produit a une image locale
ensure_product_has_image("5000112611861")

# Synchroniser toutes les images locales avec MongoDB
sync_all_images_to_mongodb()
```
...
| **Affichage images dashboard** | Les thumbnails n'apparaissent pas toujours dans index.html | UX + Validation impossible |
...
| ~~Images seed en BDD~~ | Images locales `pipeline/assets/` + chemin normalisé MongoDB (18/12/2025) |
