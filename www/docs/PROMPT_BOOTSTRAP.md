...
- `manual_leclerc_cdp.py` ouvre jusqu’à 10 cartes du listing et n’accepte une fiche que si l’EAN lu via « Informations pratiques » correspond au seed. Les négatifs imposés : `sans sucre`, `sans sucres`, `zero`, `zéro`. Ne bascule sur d’autres requêtes qu’après avoir rejeté ces 10 cartes.
- Auchan Talence Gallieni : slug officiel `auchan-drive-supermarche-talence-gallieni` (storeId `6117`). Lorsqu’un bouton « Choisir ce drive » ou « Afficher le prix » apparaît, le cliquer avant d’extraire le prix.
...
- Course U (Super U Eysines) nécessite une session CDP valide : si tu obtiens `CF_BLOCK`/`NO_PRICE`, repasse par Chrome remote (`start_chrome_debug.sh` → fermer l’overlay → `save_state_from_cdp.py courseu`). Une fois une PDP Course U validée, le fetcher réutilise l’URL stockée dans `seed_catalog.py` (`courseu_url`) pour éviter Cloudflare ; si la protection revient, repars d’un profil Chrome vierge avant de resauvegarder la state.
...
- Fournir une image locale pour chaque produit (`seed_catalog.py` → `./assets/...`).
- Avant toute collecte multi-enseigne, récupérer le **descriptif seed** :
  ...
  3. Mettre à jour `seed_catalog.py` + asset local avant de lancer les autres fetchers (Chronodrive, Intermarché, Leclerc, Monoprix, etc.).
