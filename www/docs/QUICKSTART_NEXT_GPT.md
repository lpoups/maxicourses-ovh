...
- **Cas particulier `server.py`** : après la moindre modification, exécuter immédiatement `cd maxicourses_test && pkill -f "maxicourses_test/server.py" || true && USE_CDP=1 python3 server.py` (ou équivalent) pour que l’API exposée à `index2.html` serve les nouveaux adaptateurs (Casino/Spar, etc.). Documente ce redémarrage dans le handover.
